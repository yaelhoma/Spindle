...
static int encode_packet(handshake_packet_t *packet, int secret_key, uint64_t session_id,
                          ...
                          int is_server)
{
   ...
   int secret_key = exchange_random_number(sockfd);

   /**
    * Encode socket names, session, gid, and uid into a handshake_packet_t
    **/
   debug_printf("Creating outgoing packet for handshake\n");
   result = encode_packet(&packet, secret_key, session_id, &saved_conninfo->server_addr, &saved_conninfo->client_addr);
   if (result < 0) {
   ...
   debug_printf("Encoded packet: server_port = %d, client_port = %d, "
                "uid = %d, gid = %d, session_id = %llu, signature = %lx, secret_key = %d\n",
                (int) packet.server_port, (int) packet.client_port, (int) packet.uid, (int) packet.gid,
                (unsigned long long) packet.session_id, (unsigned long) packet.signature, packet.secret_key);
   ...
   result = encode_packet(&expected_packet, secret_key, session_id, &saved_conninfo->server_addr, &saved_conninfo->client_addr);
   if (result < 0) {
                           ...
                           handshake_packet_t *recvd_packet)
{
   ...
   if (expected_packet->secret_key != recvd_packet->secret_key) {
      security_error_printf("Received handshake with different secret_key.  Expected %x, got %x\n",
                            expected_packet->secret_key, recvd_packet->secret_key);
      return HSHAKE_ABORT;
   }
...
{
   int priv_number = rand()%10;
   int temp = pow(G,priv_number);
   int val_to_send = temp % P;
   int result, key_generated, rand_received;

   debug_printf("Sending val_to_send %d generated by priv_number %d on network\n", val_to_send, priv_number);
   result = reliable_write(sockfd, &val_to_send, sizeof(val_to_send));
   if (result != sizeof(val_to_send)) {
   ...
   temp = pow(rand_received,priv_number);
   key_generated = temp % P;
